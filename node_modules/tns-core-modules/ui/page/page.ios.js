...
function isBackNavigation(frame, entry) {
    if (!frame) {
        return false;
    }
...
var UIViewControllerImpl = (function (_super) {
    ...
    UIViewControllerImpl.prototype.viewWillAppear = function (animated) {
        ...
        if (!page.modal && newEntry && (!frame || frame.currentPage !== page)) {
            var isBack = isBackNavigation(frame, newEntry);
    ...
    UIViewControllerImpl.prototype.viewDidDisappear = function (animated) {
        ...
        var frame = page.frame;
        if (!modalParent && frame && frame.backStack.length > 0 && frame._navigationQueue.length === 0 && frame.currentPage === page) {
